"""Separate read-only handoff endpoint audit. Derived from audit_saved.py with
only run/manifest/output paths adapted; complete static checks are repeated.
No minimization or dynamics.
"""
...
RUN=Path('/Users/ashujo/.cache/dynamol-research/v1-loop-release/accepted-coordinate-handoff-v1')
OUT=RUN.parent/'handoff-independent-v1';OUT.mkdir(exist_ok=True)
...
start=time.monotonic();plan=load(RUN/'plan.json');dataset=Path('/Users/ashujo/Documents/Science/DynaMol/docs/audit/loop-fallback/runs/root-final/1UA2/workspace/datasets/4dcb5c25f5c14559');parameters=load(dataset/'loop-refinement-parameters.json')
hashchecks={**plan['frozen_inputs'],**plan['implementation_sha256']}
...
for name in ['torsion-source-ccd-2027']:
 folder=RUN/'search'/name/'validation';result=load(folder/'result.json');recorded=load(folder/'reference.json');savedrecord=load(folder/'saved-reference.json')
...
save(OUT/'audit-summary.json',report);save(Path(__file__).with_name('handoff-audit-summary.json'),report)
