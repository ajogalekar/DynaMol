...
class CoreTests(unittest.TestCase):
    ...
    def test_forged_huge_npy_header_rejects_before_any_array_allocation(self):
        header=io.BytesIO();np.lib.format.write_array_header_1_0(header,
            {'descr':'<f8','fortran_order':False,'shape':(1_000_000_000,3)})
        buf=io.BytesIO()
        with zipfile.ZipFile(buf,'w') as archive:archive.writestr('x.npy',header.getvalue())
        calls=[]
        def forbidden(*args,**kwargs):calls.append((args,kwargs));raise AssertionError('Array allocation must not be attempted')
        with patch.object(np,'ndarray',forbidden):
            with self.assertRaisesRegex(ValueError,'before allocation'):c.real_arrays(buf.getvalue(),{'x':(1,3)})
        self.assertEqual(calls,[])

    def test_duplicate_npy_members_reject_before_load(self):
        import warnings
        header=io.BytesIO();np.save(header,np.ones((1,3)));buf=io.BytesIO()
        with warnings.catch_warnings():
            warnings.simplefilter('ignore',UserWarning)
            with zipfile.ZipFile(buf,'w') as archive:
                archive.writestr('x.npy',header.getvalue());archive.writestr('x.npy',header.getvalue())
        with patch.object(np,'load',side_effect=AssertionError('Must reject before NumPy load')):
            with self.assertRaisesRegex(ValueError,'Duplicate'):c.real_arrays(buf.getvalue(),{'x':(1,3)})

    def test_truncated_or_trailing_npy_payload_rejects_before_load(self):
        header=io.BytesIO();np.save(header,np.ones((1,3)));payload=header.getvalue()
        for changed in [payload[:-1],payload+b'extra']:
            buf=io.BytesIO()
            with zipfile.ZipFile(buf,'w') as archive:archive.writestr('x.npy',changed)
            with patch.object(np,'load',side_effect=AssertionError('Must reject before NumPy load')):
                with self.assertRaisesRegex(ValueError,'payload size'):c.real_arrays(buf.getvalue(),{'x':(1,3)})
