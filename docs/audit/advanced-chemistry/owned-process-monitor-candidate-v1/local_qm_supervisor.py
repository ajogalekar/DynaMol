...
def _native_owned_group_rows(group_id):
    """Read only our process group through native psutil calls after ps times out.

    Errors other than a process exiting remain fatal. An unavailable memory
    sample is never replaced with a zero or reused as a fresh measurement.
    """
    import psutil
    rows = []
    for pid in psutil.pids():
        try:
            ...
            rss = 0 if status == psutil.STATUS_ZOMBIE else process.memory_info().rss
            # Recheck ownership after querying a process which may have exited.
            if os.getpgid(pid) != group_id:
                continue
            rows.append(dict(pid=pid, pgid=group_id, rss_bytes=rss,
                             stat='Z' if status == psutil.STATUS_ZOMBIE else 'R', command=''))
...
def _group_is_alive(group_id):
    ...
    try:
        ...
        return any(not row['stat'].startswith('Z') for row in _native_owned_group_rows(group_id))
...
def resource_sample(child_pid, max_rss_bytes, running_disk_bytes, output):
    try:
        ...
        rows = _native_owned_group_rows(child_pid)
