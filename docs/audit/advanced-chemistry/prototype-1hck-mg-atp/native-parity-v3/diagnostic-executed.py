...
def main():
    ...
    changes=[];bound=0.;general_phases=[]
    for i in range(torsion.getNumTorsions()):
        ...
        if abs(p-math.pi)<=.001:
            ...
            if abs(math.cos(p))<=1e-6 or abs(math.sin(p))<=1e-6:
                raise ValueError(f'Additional native trigonometric clamp needs explicit representation: {p}')
            general_phases.append(p)
    arrays={};modes={}
    ...
    report={'scope':'Identical complete static models, three input poses, native process per pose; no dynamics or chemistry accuracy inference.',
        ...
        'general_phases_unchanged':sorted(set(general_phases)),
