...
Every new ligand records `parameter_atom_map` for **all atoms**, pairing the prepared name with the native index and XML template atom name. `IdentityForceField` verifies this map against names, elements, bonds and the absence of external bonds before assigning parameters. This prevents graph symmetry from swapping distinct native improper assignments. The same factory is used for preparation, solvent preview, saved-state reload and simulation. Earlier DynaMol bundles can recover their exact map from the recorded heavy-atom map and documented hydrogen naming; incomplete or contradictory identities fail explicitly. The adapter uses two narrow OpenMM matching hooks, covered by native force/energy, reordered topology, Modeller, repeat-copy and persistence tests.

For external reuse, load the saved preparation through `backend.prepared_system.load_prepared_forcefield`, or use an exported native System/topology with explicit particle assignments. A standalone `app.ForceField(ligand.xml)` call uses graph matching and can exchange symmetric atoms; the XML alone does not carry DynaMol's runtime identity enforcement. Native Amber ligand files, full maps and conversion evidence remain in the downloaded parameter archive. See the [6DBK conversion review](audit/ligand-conversion-fix/scientific-review.md).
...
**New simulation** starts a fresh setup: form edits, source tabs, expanded options, errors, logs and scroll position reset, and historical preparation/solvation completion banners are hidden. The current structure, its recorded preparation/solvation state, the engine choice and Background activity history remain available. Active jobs continue and reconnect to their monitor. Simulate and the background-progress shortcut reopen Studio without starting a new setup. Late responses from the previous setup cannot replace the current structure or restore a historical completion banner.
