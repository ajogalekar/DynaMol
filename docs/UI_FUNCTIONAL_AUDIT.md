...
## Follow-up: choose an engine before preparation

Simulation Studio now starts with **01 Choose your engine**, followed by **02 Start with a structure** and **03 Make it your simulation**. The choice survives closing/reopening Studio within the app session. OpenMM retains the existing pH/repair/ligand preparation and water preview. GROMACS keeps source loading and explains its separate native setup when Start is pressed, with its own force field, hydrogen defaults and solvent; OpenMM-only preparation and water actions are absent. Prepared or solvated OpenMM inputs show an early compatibility notice and explicit return-to-OpenMM action. Switching is locked during submission and active jobs; selecting the current engine preserves readiness. GROMACS readiness no longer suggests that the OpenMM target pH applies.

The final production frontend passed **28/28 Chrome browser tests**, zero retries/skips, in **56.2 seconds**. Five new cases exercise desktop/narrow engine ordering, raw-input loading and retained choice, submission settings/locks, same-engine selection, and real prepared/solvated input recovery. The same run includes 23 existing preparation-progress, source-import, fullscreen and general-control regressions. The existing backend rejection test also passed. [Per-test results and source hashes](audit/engine-preparation-order/browser-results.json), [desktop](audit/engine-preparation-order/engine-first-1440.png), [narrow view](audit/engine-preparation-order/engine-first-547.png), and [real preparation/solvation records](audit/engine-preparation-order/engine-preparation-real-results.json) retain the evidence.

The [compatibility review](audit/engine-preparation-order/scientific-review.md) distinguishes general engine interoperability from this application's unimplemented state conversion. No force-field/preparation algorithms changed and no new MD or cross-engine equivalence validation was performed. These checks validate the UI routing, not universal chemistry coverage or scientific accuracy.

## Follow-up: fresh New simulation setup

**New simulation** now resets the setup form, source tabs, expanded options, errors, logs and scroll position, and suppresses historical preparation/solvation completion banners. The loaded structure, its recorded preparation state, selected engine and saved job history remain available. Active jobs continue and reconnect. Session checks prevent delayed imports, result loads and job polling from restoring the previous setup. History's Open action shows an Opening spinner while loading coordinates.

The production frontend passed **13/13 focused Chrome browser tests**, with no retries, skips or failures, in **39.1 seconds** on a separate API and data directory. Five new cases cover real completed preparation followed by a reset, preserved provenance/defaults/history, delayed terminal polling, stale upload/coordinate/history responses, and deliberate history reopening. Eight existing preparation-progress and engine-selection cases passed alongside them, including active preparation reconnection. TypeScript and the production build passed. This change does not modify preparation chemistry or native engines.

[Per-test results and source hashes](audit/new-simulation-reset/browser-results.json) and [the fresh setup screenshot](audit/new-simulation-reset/fresh-setup.png) retain the evidence. Initial test-authoring runs encountered incorrect field selectors; the final run uses corrected accessible-role selectors. This focused follow-up does not repeat the full earlier functional audit.

## Live setup and completion update

The water stage is promoted ahead of run settings. Optional pre-run measurement picking shares the Explore editor and keeps the bottom plot across modes. Real native OpenMM and GROMACS runs verify saved-frame live tracking and checkpoint recovery; timing replays of those real outputs exercise automatic and manual completion, camera/plot persistence and protection against late responses. See [live tracking](LIVE_TRACKING.md) and [the native record](audit/live-simulation-tracking/native-validation.json). The browser suite additionally checks that atom-search Enter and all editor controls cannot submit a simulation.
