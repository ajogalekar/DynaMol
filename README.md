...
**Measure.** Click **Add a measurement** (or **Measure**) below the timeline, choose distance, angle or dihedral, then pick two, three or four atoms. The bottom panel shows selection progress and automatically adds the time plot when the selection is complete. You can cancel or retry there. For a quick inspection, select atoms directly in the inspector: the current value appears immediately, and **Plot over time** saves its plot when wanted. Pick donor–hydrogen–acceptor for a geometric hydrogen-bond trace and occupancy. Atom search supplements direct 3D picking. Plots use actual timestamps, support click-to-seek, and export CSV; torsion summaries use circular statistics. Measurements come from original saved coordinates with minimum-image periodic geometry when valid box data exists. Interpolation and display alignment do not affect the calculated values.
...
**Prepare.** Missing-atom and supported missing-loop/residue building choices are visible before the Prep button. For multichain proteins, **Use one monomer** selects the suggested protein chain (or another chain from the dropdown) and its associated molecules as a separate starting structure. The original remains available through **Restore full structure**. This selects a coordinate chain, not an inferred biological assembly; the extracted structure must be prepared again. See [repair choices and single-chain setup](docs/MONOMER_SETUP.md).
...
Inspection warns about missing sequence and backbone gaps; known internal gaps up to 12 residues can be modeled with an explicit checkbox. Incomplete ligands offer explicit modeled repair from their chemical reference or per-molecule removal, and named ions are checked against the installed force field. See [chemistry repair and viewer sizing](docs/CHEMISTRY_REPAIR.md) for the choices and current limits. Modeled loops receive bounded refinement with the complete prepared force field. Observed heavy atoms stay fixed initially; difficult closures can receive a recorded, restrained relaxation of immediate protein neighbors within a 1 Å movement limit. Ligands and protected residues stay fixed. The acceptance checks cover geometry, chirality and collisions but not backbone conformation: an independent Ramachandran check found outliers among modeled residues in 4 of the 7 accepted panel loops, so treat rebuilt loops as rough starting coordinates. See [loop repair](docs/LOOP_REPAIR.md) for the exact scope, checks and evidence. Systems with rebuilt loops must also be minimized before dynamics. Other protein-only preparation can perform backbone-restrained local relaxation; retained complexes require explicit solvent for simulation. The resulting structure, settings, ligand files and warnings are saved as a separate dataset.
...
**Solvate.** The prominent **Water & environment** step offers **Build water** after preparation, with solvent and box padding beside it. A background job builds an actual TIP3P box, displays water and ions, and saves the system. OpenMM reuses those coordinates, protonation states and ligand parameters. Update box applies the chosen padding. Prepared protein–ligand complexes require explicit water; implicit solvent remains a protein-only option. A solvent preview has not been equilibrated.

**Simulate.** Choose OpenMM or GROMACS, configure temperature, duration, solvent and optional advanced settings, then start a real local job. Optionally expand **Track during simulation** to select up to 12 distances, angles, dihedrals or hydrogen bonds before starting. Pick atoms in the scene or use atom search; the initial values appear immediately. The bottom plot remains visible in Simulate and Explore, updating from saved production frames. These are passive observations, not restraints. GROMACS currently tracks heavy-atom geometry; hydrogen tracking requires OpenMM because native GROMACS setup rebuilds hydrogens.

A followed run opens automatically in Explore when complete, preserving camera rotation/zoom and tracked plots. **Run in background**, switching molecules, or starting a new setup keeps your current scene; a completed-run notice lets you open the result later with its measurements. The live viewer shows the starting structure while plots update; completed trajectories support normal playback and click-to-seek. Cancel or resume a run, or download its files with configuration, seed, versions, provenance, and engine output. See [live tracking and completion](docs/LIVE_TRACKING.md).
...
Imports default to **250 MiB per file**, **100,000 atoms**, **10,000 retained frames**, and **256 MiB of retained coordinates**. For larger systems on a capable computer, set `DYNAMOL_MAX_ATOMS=200000` before starting the backend; 200,000 is the maximum supported atom setting and the coordinate-memory limit still applies. The browser receives the retained coordinate buffer in full. Use import stride or trim large trajectories externally. This alpha does not yet stream multi-gigabyte trajectories.
...
Automatic preparation supports **standard amino-acid proteins, SEP/TPO/PTR/HYP modifications, and supported noncovalent protein–ligand complexes**. Ligands are retained by default. Eligible missing ligand fragments can be modeled only after explicit repair selection; larger or ambiguous omissions stay blocked. An unknown chemical graph, unsupported element, covalent ligand linkage, or unassigned force-field term produces a component-specific error; source molecules are not silently removed. Other unnatural residues are recognized and retained, with a specific template blocker; this does not provide parameters for every unnatural amino acid. Nucleic acids require another preparation workflow.

The [chemistry support matrix](docs/CHEMISTRY_SUPPORT.md) records checks across all 20 standard amino-acid types, representative charge states, termini/caps, disulfides, organic ligands, ATP/ADP/NAD/FAD and named ions including Na+/Cl−/K+/Mg2+/Ca2+/Zn2+. Heme, covalent cofactors, unregistered metal states and unregistered unnatural amino acids still need specialized validated templates. Loading these structures remains separate from eligibility for MD preparation.
...
The [UI functional audit](docs/UI_FUNCTIONAL_AUDIT.md) records exercised controls, real-canvas checks, format fixtures, defects and the current browser-test results. The [five-system pressure test](docs/audit/five-system-pressure-test/REPORT.md) records native simulations, Chrome checks, fixes, and rejected preparation attempts. The [complex preparation review](docs/complex-preparation-review.md) records native ligand validation and a bounded OpenMM continuity run. The full prepared 9AX6 structure is retained, but its complete explicit-water box exceeded the 100,000-atom profile used in that audit; its documented native MD check uses one complete observed complex selected from its two copies.
