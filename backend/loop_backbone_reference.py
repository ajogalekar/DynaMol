"""Native backbone-reference check for the opt-in complete loop validator.

Numerical scoring is extracted from the independently audited research helper.
The native dependency is imported only when scoring is explicitly requested.
"""
import hashlib
...
def rama_report(topology, positions, selected=None):
    # Native reference tables are optional until the opt-in path is qualified.
